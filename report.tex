...
\acknowledgement{ This work was supported by the
  Science and Engineering Research Council, Agency of Science, Technology and
  Research, Singapore, through the National Robotics Program under Grant No.172
  25 00063. }
\maketitle
...
\begin{align}
    ...
    %\mathcal{L}(t) = \sum_{T_s} \mathbf{s}^o(t) - \sum_{T_s} \mathbf{\Tilde{s}}(t) + \gamma \mathcal{L}_a(t)
\end{align}
...
\ttnote{The test accuracies of the models are summarized in Table
  \ref{tbl:classacc325}. The tactile modality SNN model gives 12\% higher
  accuracy than vision-only modality. The multi-modal SNN model achieves the
  highest score of 81\%, an improvement of over 11\% and compared to the tactile
  modality variant.}
...
visual information \wcnote{Cite DexNet and PointnetGPD and etc}, and does not
...
%Again, this suggests that potential gains may be obtained by modifying the multi-modal SNN to optimize predictions across time.

%This analysis also  event-based attention models that may fuse modalities more effectively.

%Our analysis suggests a promising line of work in attention models for multi-modal fusion across time.

%Future work may look into combining both modalities more effectively, e.g., by encouraging the VT-SNN to emphasize different modalities at different times. 
